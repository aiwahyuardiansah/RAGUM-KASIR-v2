...
/* Render struk sebagai HTML monospace yang bisa langsung Ctrl+P
   ...
   o.store.logo (opsional): data URL gambar, dicetak di atas nama toko.
   o.store.qris (opsional): data URL gambar QRIS statis, dicetak di bawah total
                            HANYA jika metode bayar = QRIS. */
function receiptHTML(o) {
  const r = buildReceiptText(o);
  const W = r.width;
  const logo = (o.store && o.store.logo)
    ? `<div class="logo"><img src="${o.store.logo}" alt=""></div>` : '';
  // QR QRIS hanya ditampilkan saat metode bayar QRIS dan gambar tersedia
  const showQris = o.store && o.store.qris && o.payment && o.payment.method === 'QRIS';
  const qris = showQris
    ? `<div class="qris"><div class="qris-cap">SCAN UNTUK BAYAR (QRIS)</div><img src="${o.store.qris}" alt=""><div class="qris-cap">Nominal diisi sesuai TOTAL di atas</div></div>` : '';
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .logo img {
    max-width:${W.mm <= 58 ? '38mm' : '54mm'}; max-height:20mm;
    filter:grayscale(1) contrast(1.9) brightness(.92);
    image-rendering:auto;
  }
  .qris { text-align:center; margin-top:8px; }
  .qris img {
    width:${W.mm <= 58 ? '40mm' : '52mm'}; height:auto;
    filter:grayscale(1) contrast(1.4);
    image-rendering:pixelated;
  }
  .qris-cap { font-family:'Courier New',monospace; font-size:${W.font-1.5}px; margin:3px 0; }
  .receipt-text { white-space:pre; }
  .card { box-shadow:0 2px 14px rgba(0,0,0,.14); }
  @media print {
...
<div class="wrap"><div class="paper card">${logo}<div class="receipt-text">${r.text.replace(/&/g,'&amp;').replace(/</g,'&lt;')}</div>${qris}</div></div>
</body></html>`;
}
